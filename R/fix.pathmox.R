...
{ 
    ...
    elemnod <- rep(0, N)
    nv <- 0
    spli <- .fix.xexeloa(pls, DT, EXEV, type.exev, elemnod, nv, size, "pathmox")
...
#    FT.partial <- c(FT.partial, list(spli$inner.partial$F.test)) # partial
    ...
    par(mar=c(.4,.4,1,1.5))
    openplotmat()
    text(.25, .95, c("Fix Pathmox"), cex=1)
    ...
    text(x1, arrpos[1,2], paste("p.val=",round(as.numeric(best),4),sep=""), cex=.9*.7, col="blue", pos=1) 
    ## afegir categs
    varcat <- NULL
    ...
    repeat
    {
         ...
         for (nd in 1:length(nods.niv))   # p/c cuauhmaitl
         {
              ...
              } else# segueix
              {
                   ...
                   # xexeloa
                   spli <- .fix.xexeloa(x.pls, DT.set, EXEV, type.exev, elemnod, nv, size, "pathmox")    
                   best <- spli$inner.test$p.val     
...
#    FT.global[,5] <- format(FT.global[,5], scientific=FALSE)
    ...
    mox.resul <- list(MOX=MOX, FT=FT.global, candidates=FT.otras, list.nodes=list.nodes, model=model )
