...
{ 
    ...
    if (!is.null(X)) # if X available
    {
        if (is.null(pls$data))
        {
            ...
            if (nrow(X)!=nrow(EXEV))
                stop("Arguments 'X' and 'EXEV' are incompatible. Different number of rows")
        }
    } else { # if no X
        if (is.null(pls$data)) {
            ...
            if (nrow(pls$data)!=nrow(EXEV)) 
                stop("Arguments 'pls' and 'EXEV' are incompatible. Different number of rows")
        }
    }
    if (!is.data.frame(EXEV)) 
        stop("Argument 'EXEV' must be a data frame containing factors")
    for (j in 1:ncol(EXEV))
        if (!is.factor(EXEV[,j]))
            stop("One or more columns in 'EXEV' are not factors")    
    ...
    elemnod <- rep(0, N)
    nv <- 0
    spli <- .fix.xexeloa(pls, DT, EXEV, type.exev, elemnod, nv, size, "techmox")
...
#    FT.global <- rbind(FT.global, spli$inner.test)# global
    ...
    par(mar=c(.4,.4,1,1.5))
    openplotmat()
    text(.25, .95, c("Fix Techmox"), cex=1)
    text(.75, .95, c("Supervised Tree"), cex=1)
    ...
    arrpos[2,]<- straightarrow(to=elpos[3,], from=elpos[1,],
             lty=2, lwd=1, lcol="grey", arr.pos=0.6, arr.length=0)
    textellipse(elpos[1,], 0.045, 0.045, lab=c("Root",N), box.col="grey", shadow.size=0, cex=.7)    
    ## dibuix els cuauhmaitl
    for (i in 2:3)
    {
         nodlab <- c(paste("Node",nodes[i-1]), size.node[i-1])
         if (type.node[i-1]=="node") {
             textellipse(elpos[i,], 0.05, 0.03, lab=nodlab, box.col="orange",
                         shadow.size=0, cex=.7)
         } else { # "leaf"
             textrect(elpos[i,], 0.045, 0.025, lab=nodlab, box.col="green2",
                      shadow.size=0, cex=.7)
         }
    }
    ## afegir seg vars i pvals
    x1 <- (arrpos[1,1] + arrpos[2,1]) / 2
    text(x1, arrpos[1,2], as.vector(unlist(exvar[2])), cex=.7, col="blue3")
    text(x1, arrpos[1,2], paste("pv.gm=",round(as.numeric(best),4),sep=""), cex=.9*.7, col="blue", pos=1) 
    ## afegir categs
    varcat <- NULL
    ...
    for (i in 1:2)
    {
        ...
        if (posi%%2==0) {
            for (h in 1:length(seg.cat))
                text(arrpos[i,1]-0.03, arrpos[i,2]+h/55, seg.cat[h], cex=.7)
        } else {
            for (h in 1:length(seg.cat))
                text(arrpos[i,1]+0.03, arrpos[i,2]+h/55, seg.cat[h], cex=.7)
        }
    ...
    repeat
    {
         ...
         for (nd in 1:length(nods.niv))   # p/c cuauhmaitl
         {
              ...
              {
                   DT.set <- DT[indivs,]
                   x.pls <- .pls.basic(DT.set, IDM, blocks, modes, scheme, scaled, tol, iter)
                   # xexeloa
                   spli <- .fix.xexeloa(x.pls, DT.set, EXEV, type.exev, elemnod, nv, size, "techmox")    
                   best <- spli$inner.test$p.val     
                        ...
                        if (length(rig.split) <= size.limit)
{
    ...
    terminal.node[ultim] <- "yes"
                        ...
                        {                             
                             ...
                             textellipse(elpos[nodes[nv-1],], 0.05, 0.03, lab=parentlab, box.col="orange",
                                         shadow.size=0, cex=.7)
                             nodlab <- c(paste("Node",nodes[i]), size.node[i])
                             if (type.node[i]=="node") {
                                 textellipse(elpos[nodes[i],], 0.05, 0.03, lab=nodlab, box.col="orange",
                                             shadow.size=0, cex=.7)
                             } else { # "leaf"
                                 textrect(elpos[nodes[i],], 0.045, 0.025, lab=nodlab, box.col="green2",
                                          shadow.size=0, cex=.7)
                             }
                        }
                        ## afegir exvars i pval
                        x1 <- (arrpos[1,1] + arrpos[2,1]) / 2
                        text(x1, 1.15*arrpos[1,2], as.vector(unlist(exvar[ultim])), cex=.7, col="blue3")
                        text(x1, 1.15*arrpos[1,2], paste("pv.gm=",round(as.numeric(best),4),sep=""), 
                             cex=.9*.7, col="blue", pos=1) 
                        ## afegir categs
                        varcat <- NULL
...
#    FT.global[,5] <- format(FT.global[,5], scientific=FALSE)
    MOX <- data.frame( Node=nodes, Parent=parent, Depth=nodes.level, Type=type.node,  
          Terminal=terminal.node, Size=size.node, Percent=Perc, Variable=Variable, Category=Categ )
    mox.resul <- list(MOX=MOX, FT=FT.partial, candidates=FT.otras, list.nodes=list.nodes, model=model )
